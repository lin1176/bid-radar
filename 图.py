...
def analyze_risk(unit_name, project_name, text):
    ...
    """
    system_prompt = """
    你是一套“招投标大数据审计系统”。你的任务是基于历史项目库逻辑，对用户上传的招标文件进行合规性与风险性审计。
...
with col2:
    ...
    <div style="background:#F1F5F9; padding:20px; border-radius:10px; margin-top:10px;">
        <div style="display:flex; align-items:center; margin-bottom:15px;">
            ...
            <div><b>单位画像：</b><br><span style="color:#64748B; font-size:13px;">大数据基于各行业近 10 年发标记录及中标名单</span></div>
        </div>
        <div style="display:flex; align-items:center; margin-bottom:15px;">
            <span style="font-size:20px; margin-right:10px;">📉</span>
...
if submitted:
    if not tender_text or not unit_name:
        ...
        # 模拟极具科技感的加载过程
        with st.status("🔄 正在接入公共资源数据中心...", expanded=True) as status:
